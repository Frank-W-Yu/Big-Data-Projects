...
public class Multiplication {
	public static class CooccurrenceMapper extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			context.write(new Text(line[0]), new Text(value[1]));
		}
	}
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			for (Text value: values){
				if (value.toString().contains("=")){
					String[] movieRelation = value.toString().split("=");
			...
			for (Map.Entry<String, Double> elememt: relationMap.entrySet()){
				String movie = elememt.getKey();
				double relation = elememt.getValue();
				for (Map.Entry<String, Double> item: ratingMap.entrySet()){
					String userId = item.getKey();
					double rating = item.getValue();
					context.write(new Text(userId + ":" + movie), DoubleWritable(relation * rating));
				}
			}
		}
	}
